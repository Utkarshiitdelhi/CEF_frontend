    ...
    >
      <nav className="flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-3">
          <a href="#" className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full overflow-hidden flex-shrink-0 shadow-xl ring-1 ring-white/10">
              <img src={logo} alt="CEF Logo" className="w-full h-full object-cover" />
            </div>
            <div className="hidden sm:flex flex-col leading-tight">
              <span className="text-white font-extrabold text-sm">Civil & Environmental</span>
        ...
        <div className="hidden md:flex items-center gap-6">
              ...
              >
                ...
                <span
                  className={`absolute left-2 right-2 -bottom-1 h-0.5 rounded-full transition-all duration-300 bg-gradient-to-r from-cyan-300 to-blue-500 ${
                    active === item ? "opacity-100 scale-x-100" : "opacity-0 scale-x-0"
                  }`}
                />
              </a>
            ))}
          ...
          <a
            href="#join"
            className="ml-2 inline-flex items-center gap-2 bg-gradient-to-r from-cyan-400 to-blue-500 text-black px-4 py-2 rounded-full text-sm font-semibold shadow-lg transform hover:scale-105 transition-transform"
          >
            Join Us
          </a>
        </div>
      ...
      {/* Mobile Panel */}
      <div
        className={`fixed inset-0 z-40 transition-opacity md:hidden ${isMenuOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      ...
      >
        ...
        >
            ...
            {menuItems.map((item, i) => (
              <a
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors duration-150 ${
                  active === item ? "bg-white/6 text-white" : "text-white/80 hover:bg-white/5"
                }`}
          ...
          <div className="mt-6 border-t border-white/6 pt-4 flex flex-col gap-3">
            <a href="#join" className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-400 to-blue-500 text-black px-4 py-2 rounded-full text-sm font-semibold shadow-md">
              Join Us
            </a>
            <div className="flex gap-3">
              <a href="#" className="text-white/70 hover:text-white text-sm">Instagram</a>
